...
<template>
  <div class="modal-backdrop" @click.self="attemptClose">
    <div class="modal-dialog card form-theme-red" role="dialog" aria-modal="true" aria-labelledby="invite-brand-modal-title">
      <div class="modal-header">
        <h2 id="invite-brand-modal-title">Invite a Brand</h2>
        <button type="button" class="modal-close" aria-label="Close" @click="attemptClose">×</button>
      </div>

      <template v-if="!sent">
        <p class="modal-subtitle">
          Know a brand you'd love to work with? Invite them to Capybara and start a collaboration.
        </p>

        <div class="field">
          <label for="inviteBrandEmail">Brand Email</label>
          <input
            id="inviteBrandEmail"
            v-model="email"
            type="text"
 ...
 * the same pattern used by InviteUserModal.vue.
 */
.form-theme-red {
