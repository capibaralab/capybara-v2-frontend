...
const steps = [
  { key: 'account-type', label: 'Account Type', component: StepAccountType },
...
const currentIndex = computed(() => {
  const idx = steps.findIndex((step) => step.key === route.params.step)
  return idx === -1 ? 0 : idx
})
...
// and the stepper on every later screen starts counting from "Sign In".
const visibleSteps = computed(() => steps.filter((step) => step.key !== 'account-type'))
const showStepper = computed(() => steps[currentIndex.value]?.key !== 'account-type')
const visibleCurrentIndex = computed(() =>
  visibleSteps.value.findIndex((step) => step.key === steps[currentIndex.value]?.key)
)
...
function back() {
  if (currentIndex.value > 0) router.push(stepRoute(steps[currentIndex.value - 1].key))
}
...
async function handleNext() {
  ...
  if (!isLastStep.value) {
    router.push(stepRoute(steps[currentIndex.value + 1].key))
    return
  }
    ...
    <div class="onboarding-card card">
      ...
      <div class="step-actions">
        <div class="step-actions-right">
          ...
          <button v-if="isLastStep" type="button" class="btn btn-secondary" :disabled="submitting" @click="skipBilling">
            Skip & Finish
          </button>
