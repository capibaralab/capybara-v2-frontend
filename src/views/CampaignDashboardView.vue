...
const INFLUENCER_BACK_TARGETS = {
  offers: { label: 'New Suggestions', to: '/influencer/offers' },
  live: { label: 'Live Collaborations', to: '/influencer/collaborations/live' },
  completed: { label: 'Completed Collaborations', to: '/influencer/collaborations/completed' },
}
...
const isInfluencerView = computed(() => route.path.startsWith('/influencer'))

const influencerBackTarget = computed(
  () => INFLUENCER_BACK_TARGETS[route.query.from] ?? { label: 'Collaborations', to: '/influencer/collaborations' },
)

const myOffer = computed(() => {
  if (!campaign.value || !isInfluencerView.value) return null
  return campaign.value.offers.find((offer) => offer.id === route.params.offerId) ?? null
})

const myBreakdown = computed(() => {
  if (!campaign.value || !myOffer.value) return { views: 0, clicks: 0, ctr: 0, budgetSpent: 0 }
  const row = getEngagedInfluencerBreakdown([campaign.value]).find(
    (item) => item.influencerId === myOffer.value.influencerId,
  )
  return row ?? { views: 0, clicks: 0, ctr: 0, budgetSpent: 0 }
})
...
async function respond(decision) {
  ...
  try {
    await campaignsStore.respondToOffer(myOffer.value.id, decision)
  } finally {
...
const visibleOfferRows = computed(() => {
  if (!isInfluencerView.value) return offerRows.value
  return offerRows.value.filter((row) => row.status === 'approved')
})
...
<template>
  <main class="container page">
    <RouterLink
      :to="isInfluencerView ? influencerBackTarget.to : '/advertiser'"
      class="back-link"
    >
      {{ isInfluencerView ? `← Back to ${influencerBackTarget.label}` : '← Back to My Campaigns' }}
    </RouterLink>
    ...
    <template v-else>
      ...
      <div v-if="isInfluencerView && myOffer && myOffer.status === 'pending'" class="card notice">
        <p>You've been invited to collaborate on this campaign.</p>
        <div class="offer-response-actions">
          <button type="button" class="btn btn-secondary" :disabled="responding" @click="respond('declined')">
            Decline
          </button>
          <button type="button" class="btn btn-primary" :disabled="responding" @click="respond('approved')">
      ...
      <section v-if="visibleOfferRows.length" class="card section">
        <h2>{{ isInfluencerView ? 'Other Collaborators' : 'Selected Influencers' }}</h2>
        <ul class="offer-list">
          <li v-for="row in visibleOfferRows" :key="row.id" class="offer-row">
            ...
            <span v-if="!isInfluencerView" class="badge" :class="`badge-${row.status}`">{{ row.status }}</span>
          </li>
        </ul>
      </section>

      <section
        v-if="isInfluencerView ? myOffer && myOffer.status === 'approved' : campaign.metrics"
        class="card section"
      >
        <h2>Performance</h2>
        <div class="metrics">
          <template v-if="isInfluencerView">
            <div class="metric">
              <span class="metric-value">{{ formatCurrency(myBreakdown.budgetSpent) }}</span>
              <span class="metric-label">Profit</span>
            </div>
            <div class="metric">
              <span class="metric-value">{{ formatCompactNumber(myBreakdown.views) }}</span>
              <span class="metric-label">Views</span>
            </div>
            <div class="metric">
              <span class="metric-value">{{ formatCompactNumber(myBreakdown.clicks) }}</span>
              <span class="metric-label">Clicks</span>
            </div>
            <div class="metric">
              <span class="metric-value">{{ myBreakdown.ctr.toFixed(1) }}%</span>
...
.offer-response-actions .btn-primary {
