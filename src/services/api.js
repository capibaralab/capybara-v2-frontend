...
import { CATEGORY_OPTIONS, SEGMENT_OPTIONS, GENDER_OPTIONS } from './constants'
...
function generateRecommendations(campaign) {
  ...
  const preferred = (campaign.preferredInfluencerUrls || [])
    .filter(Boolean)
    .map((url) => buildPreferredInfluencer(url, campaign))
...
export async function analyzeInfluencerAudience({ targetSegments, targetGender }) {
  const segmentLabels = (targetSegments || []).includes('all')
    ? ['all ages']
    : (targetSegments || []).map((value) => SEGMENT_OPTIONS.find((opt) => opt.value === value)?.label ?? value)
  const genderLabels = (targetGender || []).map((value) => GENDER_OPTIONS.find((opt) => opt.value === value)?.label ?? value)

  const seed = hashString(`${segmentLabels.join(',')}-${genderLabels.join(',')}`)
  const engagementRate = (3.5 + (seed % 40) / 10).toFixed(1)
  const topSegment = segmentLabels[seed % segmentLabels.length] ?? 'a broad age range'
  const growth = 8 + (seed % 15)

  const findings = [
    `Your audience skews toward ${topSegment}${genderLabels.length ? ` and is most engaged among ${genderLabels.join('/')} followers` : ''}.`,
    `Average engagement rate across this audience is ${engagementRate}%, above the platform benchmark.`,
    `Follower growth in this segment is trending up ${growth}% over the last 90 days.`,
  ]
  const { data } = await mockRequest(findings, { delayMs: 1100 })
  return data
}
...
export async function createCampaign(rawPayload) {
  ...
  const campaign = {
    ...
    preferredInfluencerUrls: payload.preferredInfluencerUrls || [],
